...
class BinanceVisionDownloader:
        ...
        async with self._semaphore:
            try:
                ...
                if df is not None and not df.is_empty():
                    bars = self._to_bars(df, asset, timeframe)
                    self._store.write_bars(bars)
                    self._stats["bars"] += len(bars)
                    self._stats["downloaded"] += 1
                    logger.info("Downloaded %s: %d bars", filename, len(bars))
                else:
    ...
    def _to_bars(self, df: pl.DataFrame, asset: Asset, timeframe: Timeframe) -> list:
        """Convert DataFrame rows to Bar objects for ParquetStore."""
